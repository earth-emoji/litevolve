...
@transaction.atomic
def assign_story_character(request, pk):
...
@transaction.atomic
def scene_characters(request, pk, template_name='stories/scene_characters.html', data={}):
    scene = Scene.objects.get(pk=pk)
    act = scene.act
    story = act.story
    #story_characters = StoryCharacter.objects.filter(story=story).values('character')
    characters = Character.objects.filter(creator=request.user.profile).exclude(pk__in=scene.characters.all())
    data['act'] = act
    data['scene'] = scene
    data['story'] = story
    data['characters'] = characters
...
@transaction.atomic
def assign_scene_character(request, pk):
    ...
    if request.method == 'POST':
        ...
        message = "%s has been assigned to %s" %(character.name, scene.title)
        data = {'message': message}
        return JsonResponse(data)
    # return response
    return HttpResponse('')

def add_dialogue(request, pk, template_name='stories/dialogue_form.html', data={}):
    scene = Scene.objects.get(pk=pk)
    form = DialogueForm(request.POST or None, qs=scene.characters.all())
    if form.is_valid():
        c = form.save(commit=False)
        c.scene = scene
        c.save()
        return redirect('stories:scene-view', scene.id)
    data['scene'] = scene
    data['form'] = form
    return render(request, template_name, data)